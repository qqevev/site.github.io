...
async function loadHeroImagesFromDB() {
    try {
        ...
        if (!images || images.length === 0) {
            console.log('Нет изображений в базе, используем стандартные');
            // Если нет изображений в БД, создаем стандартные
        ...
        images.forEach(image => {
            if (image.position === 'card1') {
                updateHeroCard(1, image);
            } else if (image.position === 'card2') {
                updateHeroCard(2, image);
            }
        });
        
        // Загружаем также статистику для карточек (участники, дни, бюджет)
        await loadHeroCardStats();
        
    } catch (error) {
        console.error('Ошибка загрузки изображений героя:', error);
        // В случае ошибки используем изображения по умолчанию
